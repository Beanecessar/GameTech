...
public:
	...
	bool FindBestPath(const NavTri* start_tri, const Vector3& start_pos, const NavTri* goal_tri, const Vector3& goal_pos,NavMesh* navMesh)
	{
		ClearPath();
		...
		while (open_set.size() > 0)
		{
			...
			if (came_from[current] != NULL)
				searchHistory.push_back({ current, came_from[current] });

			//Have we made it to the goal yet?
			if (ContainsVertex(goal_tri, current))
			{
				BuildPathFromParentMap(first, current, came_from, navMesh);
...
protected:
	void BuildPathFromParentMap(NavVertex* start, NavVertex* goal, const ParentMap& map, NavMesh* navMesh)
	{
