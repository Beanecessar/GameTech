...
bool NavMesh::IsLineVaild(const Vector3 & line_start, const Vector3 & line_end)
{
	if (line_start==line_end)
	{
		return true;
	}
	typedef pair<NavTri*, float> Pair;
	vector<Pair> intersectedTris;
	Vector3 pointOnPlane;
	for (uint i = 0; i < numNavTris; ++i)
	{
		NavTri& tri = navTris[i];
...
void NavMesh::DebugDraw()
{
	...
	const Vector4 intersect_color = Vector4(1.f, 0, 0, 1.f);
	...
	for (uint i = 0; i < numNavTris; ++i)
	{
		...
		if (tri.intersect)
		{
			NCLDebug::DrawTriangle(vA, vB, vC, intersect_color);
			NCLDebug::DrawTriangle(vB, vA, vC, intersect_color);
		}
		else
		{
#endif
